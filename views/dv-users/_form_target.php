...
<div class="dv-users-form row">
    ...
    if ($user_role == 7) {
        $form = ActiveForm::begin(['id' => 'create_taeget',
                    'fieldConfig' => ['options' => ['class' => 'form-group col-md-3']],
        ]);
        if ($model->isNewRecord) {
            ?>
            <?= $form->field($model, 'manager_id')->dropDownList($managers_arr, ['prompt' => 'Select Manager'])->label(FALSE); ?>
            <?php
            echo $form->field($model, 'da_target')->textInput(['maxlength' => true])->input('number', ['placeholder' => "DA Target", 'required' => 'required', 'min' => 0])->label(false);
            echo $form->field($model, 'dm_target')->textInput(['maxlength' => true])->input('number', ['placeholder' => "DM Target", 'required' => 'required', 'min' => 0])->label(false);
            echo $form->field($model, 'incentive')->textInput(['maxlength' => true])->input('number', ['placeholder' => "Incentive", 'required' => 'required', 'min' => 0])->label(false);
        } else {
            echo $form->field($model, 'da_target')->textInput(['maxlength' => true])->input('number', ['placeholder' => "DA Target", 'required' => 'required', 'min' => 0])->label(false);
            echo $form->field($model, 'dm_target')->textInput(['maxlength' => true])->input('number', ['placeholder' => "DM Target", 'required' => 'required', 'min' => 0])->label(false);
            echo $form->field($model, 'incentive')->textInput(['maxlength' => true])->input('number', ['placeholder' => "Incentive", 'required' => 'required', 'min' => 0])->label(false);
        ...
        <?php if ($model->isNewRecord) { ?>
            <div class="form-group col-md-3">
                <select class="form-control" name="DvCourseTarget[month]" id="dvcoursetarget-month" required>
        ...
        <?php if ($model->isNewRecord) { ?>
            <div class="form-group col-md-3">
                <select class="form-control" name="DvCourseTarget[year]" id="dvcoursetarget-year" required>
        ...
        <div class="form-group col-md-12">
            <?= Html::submitButton($model->isNewRecord ? '<i class="fa fa-check"></i> Create Target' : '<i class="fa fa-pencil"></i> Update Target', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ?>
            ...
            if ($model->isNewRecord) {
                ...
                ?>
                <?= Html::a('<i class="fa fa-times"></i> Cancel', ['create_target'], ['class' => 'btn back_button btn-danger cancel_button']); ?>
            <?php } ?>
        </div>
        <?php
        ActiveForm::end();
