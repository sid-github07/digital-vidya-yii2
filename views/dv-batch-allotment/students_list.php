		    ...
		    <?php if(count($module) > 0){ 
		        ...
		        <?php foreach($module as $value){ 
		            ...
		            <tr>
		            	...
		            	<td>
		            	<?php 
	            			foreach ($common_batch_data as $key => $val) {
		            			if(in_array($value['id'],explode(',',$val['ids']))){
		            				echo $val['completed_modules'];
		            				break;
		    					}
    						}
		            	?>
...
<script>
	...
	function reset_data(){
		...
		window.location.href = "<?php echo Url::to(['dv-batch-allotment/student_list'])?>?students_id="+ids;
