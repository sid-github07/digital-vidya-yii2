...
$this->title = "Unallocated Students list";
$this->params['breadcrumbs'][] = "Unallocated Students list"; ?>
<h4> 
	<span style="color: green;"> <b> <button class="btn btn-info"> Module</button> </b> </span>
	<button class="btn btn-success"><?php echo $batchs[0]; ?></button> 
	<span style="color: green;"> <b> <button class="btn btn-info"> Date</button></b> </span>
	<button class="btn btn-success"> <?php echo $batchs[1]; ?> </button> 
	<span style="color: green;"> <b> <button class="btn btn-info"> Day</button> </b> </span> 
	<button class="btn btn-success"><?php echo $batchs[2]; ?> </button>
</h4>
 
<div class="table-responsive">
  	<?php 
	$modules_allowed = isset($filter_array['modules_allowed']) ? $filter_array['modules_allowed'] : '';
	$form = ActiveForm::begin(['id' => 'search-form', 'method' => 'get', 'action' => Url::to(['dv-batch-allotment/students_filter'])]); ?>
	<?php
		if(isset($filter_array['students_id'])){
			$students_id = $filter_array['students_id'];
		}else{
			$students_id = isset($_GET['students_id']) ? implode(',', $_GET['students_id']) : '';
		}
	 ?>
	<input type="hidden" name="students_id" id="students_id" value="<?php echo $students_id; ?>">
    <input type="hidden" name="batch_allotment_details" id="batch_allotment_details" value="<?php echo isset($batch_allotment_details) ? $batch_allotment_details : ''; ?>">
 	<input type="hidden" name="completed_modules_name" id="completed_modules_name" value="<?php echo isset($completed_modules_name) ? $completed_modules_name : ''; ?>">
    <div class="form-group col-md-3" data-toggle="tooltip" data-placement="top" title="Module Allowed">
       <select class="form-control" name="modules_allowed">
            <option value="">Module Allowed</option>
            <?php for($i = 1 ; $i<=6 ; $i++){ ?>
                <option <?php echo $modules_allowed == $i && $modules_allowed != '' ? 'selected="selected"' : ''; ?> value="<?php echo $i; ?>"><?php echo $i; ?></option>
            <?php } ?>
        </select>
    </div>
    ...
    <div class="form-group col-md-3" data-toggle="tooltip" data-placement="top" title="Opted Day">
  	...
  	<div class="form-group col-md-3">
        <?= Html::submitButton( '<i class="fa fa-search"></i> Search' , ['class' => 'btn btn-sm pull btn-success search_submit','data-toggle'=>"tooltip" ,'data-placement'=>"top" ,'title'=>"Search"]) ?>
