...
<div class="table-responsive">
	...
	<input type="hidden" name="bybtn" id="bybtn" value="1">
	<input type="hidden" name="students_id" id="students_id" value="<?php echo $students_id; ?>">
	<div class="form-group col-md-2">
		<label>Core Modules</label>
		<?php $only_core_modules = ArrayHelper::map(DvModuleModel::find()->where(['category_type'=>'Core'])->all(),'id','module_name'); ?>
		<select class="form-control" name="core_modules" id="core_modules" multiple="multiple" size = 5>
    		<option value="">-Core Module-</option>
    		<?php foreach ($only_core_modules as $key => $value) { ?>
		    ...
		    <?php if(count($module) > 0){ 
		        ...
		        <?php foreach($module as $value){ 
		            ...
		            <tr>
		            	...
		            	<td><?php echo rtrim($unallocated_special_batch[$value['id']],','); ?>
		            	</td>
		            	<td><?php 
...
<?php $form = ActiveForm::begin(['method' => 'post', 'id'=>'submit_form','action' => Url::to(['dv-batch-allotment/display_special_batch_list'])]);?>
		<input type="hidden" name="total_selected" id="total_students_frm" value="">
		<input type="hidden" name="batch_allotment_details" id="batch_allotment_details_frm" value="<?php echo $batchs; ?>">
...
<script>
	$(document).ready(function(){
		...
		$('#submit_btn').click(function(){
 			if($('#available_batch_opt').val().trim() !='' || $('#email_search').val().trim()!=''){
 				return true;
 			}else{
 				return false;
 			}
